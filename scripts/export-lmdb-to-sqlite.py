...
add_account = (
    ...
    "block_count, confirmation_height, confirmation_height_frontier) VALUES (:account,"
    ":frontier, :open_block, :representative_block, :balance,"
    ":modified_timestamp, :block_count, :confirmation_height,"
...
if os.path.exists(db_filename):
    print("Output db already exists, removing")
    ...
    with env.begin() as txn:
        ...
        for key, value in cursor:
            ...
            if btype == Nanodb.EnumBlocktype.open:
                ...
                data_block["previous"] = block.block_value.block.previous.hex().upper()
            ...
            if btype == Nanodb.EnumBlocktype.state:
                ...
                data_block["link"] = block.block_value.block.destination.hex().upper()
                data_block["link_account"] = nanolib.accounts.get_account_id(
                    prefix=nanolib.AccountIDPrefix.NANO,
                ...
                try:
                    ...
                    ltype = linked_block_value.block_type
                    if ltype == Nanodb.EnumBlocktype.send:
                        linked_account = linked_block_value.block_value.sideband.account
                    else:
                        linked_account = linked_block_value.block_value.block.account
            ...
            data_block["signature"] = block.block_value.block.signature.hex().upper()
            data_block["work"] = hex(block.block_value.block.work)[2:]

            try:
                confirmation_value = txn.get(account, default=None, db=confirmation_db)
                confirmation_valstream = KaitaiStream(io.BytesIO(confirmation_value))
                height_info = Nanodb.ConfirmationHeightValue(
                    confirmation_valstream, None, Nanodb(None)
                )
            ...
            if data_block["height"] > 1:
                ...
                data_block["amount"] = str(abs(int(previous_balance) - int(balance)))
            else:
